    ...
    while(total_steps < hyperParams.TRAINING_FRAMES):
        ...
        while True:
            ...
            if done or steps > hyperParams.MAX_STEPS:
                ...
                ep += 1 
                break
        print("\rStep: {}, ep: {}, eps: {:.2f}, max q: {:.2f}, Average of last 100: {:.2f}".format(total_steps, ep, agent.epsilon, sum(agent.tab_max_q)/max(len(agent.tab_max_q), 1),\
        tab_mean_rewards[-1]), end="")
